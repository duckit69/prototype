...
simulate_S14.py
==============
Hardware-in-the-loop simulation for Subject S14.

Pipeline:
  1.  Connect to Omnikey reader via pyscard (smart-card I/O).
  2.  Train a generic logistic regression (LOSO – all subjects except S14).
  3.  Write 64 bytes (15 weights × 4 B + bias 4 B) to the smart card
      (simulated here as a local text file: simulations/smart_card_S14.txt).
...
Usage:
    python simulations/simulate_S14.py [--port /dev/ttyUSB0] [--dry-run]
"""
